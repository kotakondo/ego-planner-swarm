...
void EGOReplanFSM::init(ros::NodeHandle &nh, ros::NodeHandle &nh1)
{
  ...
  /*  fsm param  */
  nh_.param("fsm/flight_type", target_type_, -1);
  nh_.param("fsm/thresh_replan_time", replan_thresh_, -1.0);
  nh_.param("fsm/thresh_no_replan_meter", no_replan_thresh_, -1.0);
  nh_.param("fsm/planning_horizon", planning_horizen_, -1.0);
  nh_.param("fsm/planning_horizen_time", planning_horizen_time_, -1.0);
  nh_.param("fsm/emergency_time", emergency_time_, 1.0);
  ...
  {
    nh_.param("fsm/waypoint" + to_string(i) + "_x", waypoints_[i][0], -1.0);
    nh_.param("fsm/waypoint" + to_string(i) + "_y", waypoints_[i][1], -1.0);
    nh_.param("fsm/waypoint" + to_string(i) + "_z", waypoints_[i][2], -1.0);
  }

  /* initialize main modules */
  visualization_.reset(new PlanningVisualization(nh_));
  planner_manager_.reset(new EGOPlannerManager);
  planner_manager_->initPlanModules(nh_, visualization_);
  ...
  if (planner_manager_->pp_.drone_id >= 1)
  {
    ...
    swarm_trajs_sub_ = nh_.subscribe(sub_topic_name.c_str(), 10, &EGOReplanFSM::swarmTrajsCallback, this,
  ...
  // Previous code
  // broadcast_bspline_sub_ = nh_.subscribe("planning/broadcast_bspline_to_planner", 100,
  // &EGOReplanFSM::BroadcastBsplineCallback, this, ros::TransportHints().tcpNoDelay());

  // Comm delay introduced code
  ...
  {
    waypoint_sub_ = nh_.subscribe("/move_base_simple/goal", 1, &EGOReplanFSM::waypointCallback, this);
  }
  else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
  {
    trigger_sub_ = nh_.subscribe("/traj_start_trigger", 1, &EGOReplanFSM::triggerCallback, this);
...
void EGOReplanFSM::CommDelayBroadcastBsplineCallback(const traj_utils::BsplinePtr &msg)
{
  ...
  ros::Timer alltrajs_timer =
      nh1_.createTimer(ros::Duration(commdelay_), &EGOReplanFSM::BroadcastBsplineCallback, this, true);
